...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function GalgamePlayer(props) {
      ...
      var setTick = tick[1];
      var eng = React.useRef({ bg: null, sprites: {}, locked: {}, bgm: null, bgmVol: 1, backlog: [], vars: {} });
      ...
      function syncSprites(speaker) {
        ...
        for (var name in game[0].characters) {
          ...
          if (want) {
            ...
            eng.current.sprites[name] = { url: (c && c.url) || null, pos: (c && c.pos) || 'center', visible: true, name: name };
          } else if (cur) {
        ...
        while (i >= 0 && i < nodes.length && guard++ < 8000) {
          ...
          if (n.type === 'sprite') {
            if (n.action === 'hide') { delete eng.current.locked[n.char]; if (eng.current.sprites[n.char]) eng.current.sprites[n.char].visible = false; }
              ...
              var c = game[0].characters && game[0].characters[n.char];
              eng.current.sprites[n.char] = { url: (c && c.url) || null, pos: n.pos || (c && c.pos) || 'center', visible: true, name: n.char };
            }
            i++; continue;
          }
          if (n.type === 'bgm') {
      ...
      function startGame() { setPhase('play'); setAuto(false); eng.current.vars = {}; eng.current.sprites = {}; eng.current.locked = {}; eng.current.bgmVol = 1; runFrom(0); }
      ...
      function saveToSlot(n) {
        try {
          var saves = readSaves();
          saves[String(n)] = { idx: idxRef.current, bg: eng.current.bg, sprites: eng.current.sprites, locked: eng.current.locked, bgm: eng.current.bgm, bgmVol: eng.current.bgmVol, backlog: eng.current.backlog, vars: eng.current.vars, time: Date.now(), label: (line[0] && line[0].text ? line[0].text.slice(0, 12) : '') };
      ...
      var cgLayer = cgState[0] ? React.createElement('div', { className: 'gg-cg', onClick: closeCg },
        cgState[0].url && /\.(mp4|webm|ogg|m4a)$/i.test(cgState[0].url)
