...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function GalgamePlayer(props) {
        ...
        while (i >= 0 && i < nodes.length && guard++ < 8000) {
          ...
          if (n.type === 'sprite') {
            if (n.action === 'hide') { delete eng.current.locked[n.char]; if (eng.current.sprites[n.char]) eng.current.sprites[n.char].visible = false; }
              ...
              eng.current.sprites[n.char] = { url: (c2 && c2.url) || null, pos: n.pos || (c2 && c2.pos) || 'center', visible: true, name: n.char };
            }
            i++; continue;
          }
          if (n.type === 'face') {
      ...
      function startGame() { setPhase('play'); setAuto(false); eng.current.vars = {}; eng.current.sprites = {}; eng.current.locked = {}; eng.current.faces = {}; eng.current.bgmVol = 1; runFrom(0); }
      ...
      function saveToSlot(n) {
        try {
          var saves = readSaves();
          saves[String(n)] = { idx: idxRef.current, bg: eng.current.bg, sprites: eng.current.sprites, locked: eng.current.locked, faces: eng.current.faces, bgm: eng.current.bgm, bgmVol: eng.current.bgmVol, backlog: eng.current.backlog, vars: eng.current.vars, time: Date.now(), label: (line[0] && line[0].text ? line[0].text.slice(0, 12) : '') };
