...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function GalgamePlayer(props) {
      ...
      var setTick = tick[1];
      var eng = React.useRef({ bg: null, sprites: {}, locked: {}, hidden: {}, faces: {}, positions: {}, bgm: null, bgmVol: 1, backlog: [], vars: {} });
      ...
      function syncSprites(speaker) {
        ...
        var autoOn = (speaker && game[0].characters && game[0].characters[speaker] && !eng.current.hidden[speaker]) ? {} : {};
        if (speaker && game[0].characters && game[0].characters[speaker] && !eng.current.hidden[speaker]) autoOn[speaker] = true;
      ...
      function runFrom(start) {
        ...
        while (i >= 0 && i < nodes.length && guard++ < 8000) {
          ...
          if (n.type === 'sprite') {
            ...
            else if (n.action === 'leave') { delete eng.current.locked[n.char]; eng.current.hidden[n.char] = true; if (eng.current.sprites[n.char]) eng.current.sprites[n.char].visible = false; }
            else {
              eng.current.locked[n.char] = true;
              eng.current.hidden[n.char] = false;
      ...
      function startGame() { setPhase('play'); setAuto(false); eng.current.vars = {}; eng.current.sprites = {}; eng.current.locked = {}; eng.current.hidden = {}; eng.current.faces = {}; eng.current.positions = {}; eng.current.bgmVol = 1; runFrom(0); }
      ...
      function saveToSlot(n) {
        try {
          var saves = readSaves();
          saves[String(n)] = { idx: idxRef.current, bg: eng.current.bg, sprites: eng.current.sprites, locked: eng.current.locked, hidden: eng.current.hidden, faces: eng.current.faces, positions: eng.current.positions, bgm: eng.current.bgm, bgmVol: eng.current.bgmVol, backlog: eng.current.backlog, vars: eng.current.vars, time: Date.now(), label: (line[0] && line[0].text ? line[0].text.slice(0, 12) : '') };
      ...
      function loadFromSlot(n) {
        ...
        eng.current.hidden = (save.hidden && typeof save.hidden === 'object') ? save.hidden : {};
