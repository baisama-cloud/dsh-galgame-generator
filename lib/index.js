...
import { GALGAME_PRESET_ID, GALGAME_PRESET_DIRNAME, GALGAME_PRESET_FILES } from './galgame-preset.js'

export const name = 'dsh-galgame-generator'
export const inject = ['tools']

// Absolute package root (…/node_modules/dsh-galgame-generator), resolved from this
// module file so the preset template can be read back from the installed tree.
...
function parseScript(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (mcmd) {
      ...
      if (verb === 'hide' || verb === '隐藏' || verb === '取消' || verb === '取消显示' || verb === '移除' || verb === 'remove' || verb === 'erase') { push({ type: 'sprite', char: arg, action: 'hide', line: lineNo }); continue }
      if (verb === '退场' || verb === '离开' || verb === 'leave' || verb === 'exit') { push({ type: 'sprite', char: arg, action: 'leave', line: lineNo }); continue }
...
async function collectAssets(fs, scriptDir, signal) {
  ...
  for (const e of rootEntries) {
    ...
    // 只扫描约定素材目录（及常见别名），跳过 demo 等无关目录，避免把无关素材内嵌进独立网页
    const dname = String(e.name).toLowerCase()
    if (!/^(sprites?|images?|imgs?|bg|backgrounds?|characters?|audio|music|assets?|img_human|img_bg|img_cg|立绘|背景|音频|音乐)$/.test(dname)) continue
...
function bytesToBase64(bytes) {
  // Manual base64 (pure ASCII concatenation). Do NOT use btoa(String.fromCharCode(...)):
  // in the DSH host sandbox btoa treats the string as UTF-8, so every byte >= 0x80
  // gets re-encoded to two bytes and binary assets (jpg/png) corrupt.
  ...
  while (i < len) {
    ...
    out += CH[b0 >> 2]
    out += CH[((b0 & 3) << 4) | (b1 === -1 ? 0 : b1 >> 4)]
    if (b1 === -1) { out += '=='; break }
    out += CH[((b1 & 15) << 2) | (b2 === -1 ? 0 : b2 >> 6)]
    if (b2 === -1) { out += '='; break }
    out += CH[b2 & 63]
  }
  return out
...
(function () {
  ...
  var S = { phase: 'title', idx: 0, vars: {}, bg: null, sprites: {}, locked: {}, hidden: {}, faces: {}, positions: {}, bgm: null, bgmVol: 1, backlog: [], line: null, typed: '', done: true, choices: null, ending: '', cgState: null, auto: false, fast: false, muted: false };
  ...
  function saveToSlot(n) {
    try {
      var sv = readSaves();
      sv[String(n)] = { idx: S.idx, bg: S.bg, sprites: S.sprites, locked: S.locked, hidden: S.hidden, faces: S.faces, positions: S.positions, bgm: S.bgm, bgmVol: S.bgmVol, backlog: S.backlog, vars: S.vars, time: Date.now(), label: (S.line && S.line.text ? S.line.text.slice(0, 12) : '') };
  ...
  function loadFromSlot(n) {
    ...
    if (!save) return;
    S.bg = save.bg || null; S.sprites = save.sprites || {}; S.locked = save.locked || {}; S.hidden = save.hidden || {}; S.faces = save.faces || {}; S.positions = save.positions || {}; S.bgm = save.bgm || null; S.bgmVol = save.bgmVol || 1;
  ...
  function syncSprites(speaker) {
    var autoOn = {};
    if (speaker && GAME.characters && GAME.characters[speaker] && !S.hidden[speaker]) autoOn[speaker] = true;
  ...
  function runFrom(start) {
    ...
    while (i >= 0 && i < nodes.length && guard++ < 8000) {
      ...
      if (n.type === 'sprite') {
        ...
        else if (n.action === 'leave') { delete S.locked[n.char]; S.hidden[n.char] = true; if (S.sprites[n.char]) S.sprites[n.char].visible = false; }
        else { S.locked[n.char] = true; S.hidden[n.char] = false; var c2 = variantOf(n.char); S.sprites[n.char] = { url: (c2 && c2.url) || null, pos: n.pos || S.positions[n.char] || (c2 && c2.pos) || 'center', visible: true, name: n.char }; }
        i++; continue;
      }
      if (n.type === 'pos') {
  ...
  function startGame() { S.phase = 'play'; S.auto = false; S.vars = {}; S.sprites = {}; S.locked = {}; S.hidden = {}; S.faces = {}; S.positions = {}; S.bgmVol = 1; runFrom(0); }
...
/**
 * Install the "Galgame 生成模式" agent preset.
 *
 * DSH's native agent-preset picker (new-session chip + Settings → Agent Presets)
 * is the in-GUI mode switch, and a preset mounts a `persona` row that injects the
 * generation guidance into the model (prompt injection). On first load we write
 * the preset into the harness-home user root (`~/.dsh/.agent-presets/galgame/`)
 * so "install plugin → mode is selectable". We never overwrite a preset the user
 * has already made their own: only write when the target file is absent or still
 * byte-identical to a previous install of ours.
 * @returns {Promise<{installed:boolean,presetId:string,dir:string} | {error:string}>}
 */
async function installPreset() {
  try {
    const dir = join(dshHomePath('.agent-presets'), GALGAME_PRESET_ID)
    await mkdir(dir, { recursive: true })
    let installed = false
    for (const name of GALGAME_PRESET_FILES) {
      const source = join(PACKAGE_ROOT, GALGAME_PRESET_DIRNAME, name)
      let content
      try {
        content = await readFile(source, 'utf8')
      } catch (e) {
        // Template missing from this install — nothing we can install.
        return { error: 'preset template missing: ' + source, presetId: GALGAME_PRESET_ID, dir }
      }
      ...
      try {
        ...
        // treat ENOENT as "not present"; any other read error leaves prev === null
        if (!e || e.code !== 'ENOENT') prev = '__unreadable__'
      }
      if (prev === null) {
        // Cleanly absent → install our template.
        await writeFile(target, content, 'utf8')
        installed = true
      } else if (prev === content) {
        // Already our current version → nothing to do.
      } else if (prev !== '__unreadable__') {
        // Present but user-modified → leave it alone; the user owns this preset now.
        // (Preset directory exists, so discovery will list it as "ready".)
      }
      // prev === '__unreadable__' → leave untouched; do not clobber or guess.
    }
    return { installed, presetId: GALGAME_PRESET_ID, dir }
  } catch (e) {
...
export function apply(ctx) {
  ...
  // Make the "Galgame 生成模式" agent preset available (best-effort; never fails load).
  installPreset().then((res) => {
    if (res && res.error) ctx.logger?.warn?.('galgame preset install failed: %s', res.error)
    else if (res && res.installed) ctx.logger?.info?.('galgame 生成模式 installed at %s', res.dir)
  }).catch(() => {})
