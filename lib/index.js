...
const AUD_EXT = ['mp3', 'ogg', 'wav', 'm4a', 'mp4', 'flac']
...
function parseScript(text) {
  ...
  const result = { title: '', intro: '', characters: {}, backgrounds: {}, labels: {}, nodes: [], warnings: [], errors: [] }
  let section = null
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (section === 'char') {
      ...
      else result.warnings.push('第' + lineNo + '行：角色行格式应为「名字: 立绘文件.png」，已忽略')
      continue
    }
    if (section === 'bg') {
      const m = /^(.+?)\s*[:：=]\s*(.+)$/.exec(trimmed)
  ...
  for (const e of rootEntries) {
    ...
    for (const se of subEntries) {
      if (se.type === 'file') pushFile(joinPath(sub, se.name), se.name)
        ...
        if (!/^(sprites?|images?|imgs?|bg|backgrounds?|characters?|audio|music|assets?|img_human|img_bg|立绘|背景|音频|音乐)$/.test(lname)) continue
...
(function () {
  ...
  var S = { phase: 'title', idx: 0, vars: {}, bg: null, sprites: {}, locked: {}, bgm: null, bgmVol: 1, backlog: [], line: null, typed: '', done: true, choices: null, ending: '', auto: false, fast: false, muted: false };
  ...
  function saveToSlot(n) {
    try {
      var sv = readSaves();
      sv[String(n)] = { idx: S.idx, bg: S.bg, sprites: S.sprites, locked: S.locked, bgm: S.bgm, bgmVol: S.bgmVol, backlog: S.backlog, vars: S.vars, time: Date.now(), label: (S.line && S.line.text ? S.line.text.slice(0, 12) : '') };
  ...
  function loadFromSlot(n) {
    ...
    if (!save) return;
    S.bg = save.bg || null; S.sprites = save.sprites || {}; S.locked = save.locked || {}; S.bgm = save.bgm || null; S.bgmVol = save.bgmVol || 1;
  ...
  function syncSprites(speaker) {
    ...
    for (var name in GAME.characters) {
      ...
      if (want) {
        var c = GAME.characters[name];
        S.sprites[name] = { url: (c && c.url) || null, pos: (c && c.pos) || 'center', visible: true, name: name };
      } else if (cur) {
  ...
  function runFrom(start) {
    ...
    while (i >= 0 && i < nodes.length && guard++ < 8000) {
      ...
      if (n.type === 'sprite') {
        ...
        else { S.locked[n.char] = true; var c2 = GAME.characters[n.char]; S.sprites[n.char] = { url: (c2 && c2.url) || null, pos: n.pos || (c2 && c2.pos) || 'center', visible: true, name: n.char }; }
        i++; continue;
      }
  ...
  function startGame() { S.phase = 'play'; S.auto = false; S.vars = {}; S.sprites = {}; S.locked = {}; S.bgmVol = 1; runFrom(0); }
...
async function buildGame(fs, scriptPath, signal) {
  ...
  for (const name of Object.keys(parsed.characters)) {
    const c = parsed.characters[name]
    const relId = resolveRef(c.file, scan)
    if (!relId) parsed.warnings.push('角色「' + name + '」的立绘文件找不到：' + c.file)
    addUsed(relId)
    characters[name] = { name, file: c.file, pos: c.pos, url: assetUrl(relId), found: !!relId }
  }
  const backgrounds = {}
  ...
  for (const n of parsed.nodes) {
    if (n.type === 'bgm' && n.file) {
      const relId = resolveRef(n.file, scan)
      if (!relId) parsed.warnings.push('音乐文件找不到：' + n.file + '（第' + n.line + '行）')
      n.file = relId || n.file
    }
  }
  const assetUrls = {}
    ...
    execute: async (args, exec) => {
      ...
      try {
        ...
        return {
          ...
          stats: game.stats,
          characters: Object.keys(game.characters).map((k) => ({ name: game.characters[k].name, file: game.characters[k].file, found: game.characters[k].found })),
